...
endpoint (LM Studio, Ollama, etc.) and writes a short topic plus a 2–3
sentence topic_long back into each session.
...
def _parse_indexed_topics(text: str, expected: int) -> dict[int, dict[str, str]]:
    ...
    1. ``[{"idx":0,"topic":"…","topic_long":"…"}, …]`` — bare array (default).
    2. ``{"results": [ … ]}`` — some LM Studio structured-output modes wrap
       the array in a top-level object.
    3. ``[<topic-string>, …]`` — positional string fallback (legacy).

    Strips ``<think>…</think>`` blocks that reasoning models sometimes emit
    inline and code-fence wrappers (``` ```json ```) before parsing.
    """
    if not text:
        return {}
    # Strip chain-of-thought blocks (Qwen3, DeepSeek-R1, …) that may leak
...
def extract_topics(sessions: list[dict], cfg: dict,
    ...
    """Annotate sessions in place with ``topic`` (and ``topic_long``).

    Returns the number of sessions whose topic was set.
    ...
    for chunk_start in range(0, len(candidates), batch_size):
        ...
        try:
            ...
            try:
                ...
                body_snippet = ""
            # LM Studio occasionally returns HTTP 400 {"error":"Model unloaded."}
            # right after a crash/auto-unload while JIT is mid-reload. The next
            # request normally succeeds, so do one bounded retry.
            if e.code == 400 and "unloaded" in body_snippet.lower():
                log.info("topic_llm: model unloaded, retrying batch in 5s")
                time.sleep(5)
                try:
                    resp = _post_json(endpoint, payload, timeout)
                except Exception as e2:
                    log.warning("topic_llm retry failed: %s", e2)
                    continue
            else:
                log.warning("topic_llm HTTP %s: %s | body=%s",
        ...
        for i, (sess, brief) in enumerate(batch):
            ...
            topic = (entry.get("topic") or "").strip().strip('"').strip("'")
            topic_long = (entry.get("topic_long") or "").strip().strip('"').strip("'")
            if not topic:
                continue
